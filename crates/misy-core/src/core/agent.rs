...
use crate::{
    Message, MessageRole, ModelRef, ProviderError, ProviderEvent, ProviderId, ToolCall,
    ToolRegistry,
};
...
impl CoreState {
    ...
    async fn start_and_collect_turn(
        ...
        *active
        ...
        let stream = self
            .collect_stream(submission, request_id, active, events, pending)
            .await;
        *active
            .request
            .lock()
            .expect("active request mutex must not be poisoned") = None;
        stream.map(TurnStream::into_model_turn)
    }
    ...
    async fn collect_stream(
        ...
        active: &ActiveSubmission,
        events: &mut mpsc::UnboundedReceiver<ProviderEvent>,
        pending: crate::PendingProviderRequest,
        ...
        loop {
            ...
            tokio::select! {
                result = async { response.as_mut().expect("response future is present").await }, if response.is_some() => {
                    stream.record_response(result)?;
                    response = None;
                }
                event = events.recv() => match event {
                    Some(event) => self.consume_stream_event(event, request_id, submission, &mut stream)?,
