...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"auth.complete"'*) case "$line" in *'"id":"pending-a"'*) sleep 2; reply '{"credentials":{"type":"oauth","access":"opaque"}}' ;; *'"code":"expired"'*) reply '{"credentials":{"type":"oauth","access":"stale","refresh_token":"rotate","expires_at":0}}' ;; *'"id":"fixture-session"'*|*'"code"'*) reply '{"credentials":{"type":"oauth","access":"opaque"},"nested":{"credentials":{"access":"complete-secret"}}}' ;; *) printf '{"jsonrpc":"2.0","id":%s,"error":{"code":400,"message":"missing opaque auth session"}}\n' "$id" ;; esac ;;
    *'"method":"auth.refresh"'*) sleep 1; reply '{"credentials":{"type":"oauth","access":"refreshed-opaque","expires_at":4102444800000},"nested":{"credentials":{"access":"refresh-secret"}}}' ;;
    *'"method":"auth.logout"'*) reply '{}' ;;
    *'"method":"models.list"'*) case "$target" in *slow-models*|*slow-bad-models*) sleep 1 ;; esac; case "$target" in *bad-models*) reply '{}' ;; *explicit-default*) reply '{"models":[{"id":"fixture-model","display_name":"Fixture","context_window":4096},{"id":"fixture-model-b","display_name":"Fixture B","context_window":4096}],"default_model":"fixture-model-b"}' ;; *) reply '{"models":[{"id":"fixture-model","display_name":"Fixture","context_window":4096},{"id":"fixture-model-b","display_name":"Fixture B","context_window":4096}]}' ;; esac ;;
    *'"method":"chat.start"'*)
      ...
      case "$line" in
        ...
        *'"content":"refresh-before-chat"'*) case "$line" in *'"access":"refreshed-opaque"'*) text refreshed; complete; reply '{}' ;; *) printf '{"jsonrpc":"2.0","method":"failed","params":{"request_id":%s,"message":"expired credentials were not refreshed"}}\n' "$id"; reply '{}' ;; esac ;;
        *'"content":"credential-chat"'*) case "$line" in *'"credentials":{'*'"type":"oauth"'*'"access":"opaque"'*|*'"credentials":{'*'"access":"opaque"'*'"type":"oauth"'*) text authenticated; complete; reply '{}' ;; *) printf '{"jsonrpc":"2.0","method":"failed","params":{"request_id":%s,"message":"missing chat credentials"}}\n' "$id"; reply '{}' ;; esac ;;
