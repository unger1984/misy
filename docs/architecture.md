...
- The Rust core owns normalized state, configuration, credentials, the persisted model catalog
  cache (`~/.misy/models.json`), the in-memory conversation, agent/tool iteration, cancellation,
  provider supervision, and public events. Clients read the model cache through the core. The core
  also owns capability negotiation, credential injection, deadlines, and validation for
