...
use misy::{
    ...
    tui::{TranscriptRow, TuiClient, UiKey, UiMode},
};
...
#[test]
fn model_popup_is_centered_and_capped_on_a_wide_terminal() {
    let (_temporary, core) = core_with_providers(&[("fixture", "Fixture AI", "good-models")]);
    populate_cached_models(&core, &["fixture"]);
    let mut client = TuiClient::new(core, RecordingBrowser::default());
    client.handle_input("/model").expect("open cached models");
...
#[test]
fn model_popup_wraps_provider_tabs_into_multiple_rows() {
    let provider_ids = [
        "provider-01",
        "provider-02",
        "provider-03",
    ...
    populate_cached_models(&core, &provider_ids);
    let mut client = TuiClient::new(core, RecordingBrowser::default());
    client.handle_input("/model").expect("open cached models");
...
#[test]
fn model_popup_scrolls_models_within_its_actual_content_height() {
    let provider_ids = [
        "provider-01",
        "provider-02",
        "provider-03",
...
#[test]
fn closing_model_popup_redraws_the_underlying_screen_without_ghost_cells() {
    let (_temporary, core) = core_with_providers(&[("fixture", "Fixture AI", "good-models")]);
    populate_cached_models(&core, &["fixture"]);
    let mut client = TuiClient::new(core, RecordingBrowser::default());
...
fn populate_cached_models(core: &misy::MisyCore, provider_ids: &[&str]) {
