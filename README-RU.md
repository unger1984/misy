...
- версионированный протокол провайдеров JSON-RPC 2.0 с поиском манифестов, ленивым запуском
  процессов, потоковым чатом, браузерной, device- и prompt-авторизацией, обновлением учётных данных,
  поиском моделей, отменой запросов и опциональной нормализованной статистикой лимитов;
- встроенные плагины OpenAI, Anthropic, Kimi и API-key-каталога AnyModel, написанные на TypeScript
  и запускаемые через Bun;
- локальные Rust-инструменты `list_directory`, `read_file`, `write_file` и единый `exec_command` с
  проверкой JSON Schema, автоматическим переходом долгих команд в фон, опциональным PTY на
  macOS/Linux, интерактивным вводом через `write_stdin` и ограниченным ordered-выводом; `read_file`
  поддерживает постраничное чтение строк с однобазовыми `offset` и `limit` в пределах читаемого
  префикса размером 4 МиБ;
...
Это фундамент MVP, а не стабильный релиз. Один процесс соответствует одной активной корневой
сессии. TUI поддерживает prompt-авторизацию protocol v2 и маскирует секретные поля.
...
[Anthropic](plugins/providers/anthropic/README.md), [Kimi](plugins/providers/kimi/README.md) и
[AnyModel](plugins/providers/anymodel/README.md).
